...
export default function Header() {
  ...
  const toggleTheme = () => {
    ...
    try { localStorage.setItem('goability-theme', next); } catch (e) {}
  };
        ...
        <div className="flex items-center h-16 md:h-20">
          ...
          <div className="flex-[1_1_0] min-w-0 flex items-center justify-end gap-1 md:gap-2">
            <button onClick={toggleTheme}
              className={`p-2 rounded-lg transition-colors flex-shrink-0 ${scrolled ? 'text-gray-600 hover:bg-gray-100 hover:text-ocean-500' : 'text-white/80 hover:text-white hover:bg-white/10'}`}
              aria-label={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'}>
              {theme === 'dark' ? (
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.36 6.36l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
              ) : (
